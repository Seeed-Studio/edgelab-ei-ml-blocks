# Edgelab edgelab-yolov5 transfer learning model for Edge Impulse

This repository is an example on how to [bring your own model](https://docs.edgeimpulse.com/docs/adding-custom-transfer-learning-models) into Edge Impulse. This repository is using EdgeLab edgelab-yolov5 (an reading detection model).
...
1. Convert the training data / training labels into Edgelab edgelab-yolov5 format using [extract_dataset.py](extract_dataset.py).
1. Train edgelab-yolov5 model (using https://github.com/Seeed-Studio/Edgelab).
1. Convert the edgelab-yolov5 model into TFLite format.
1. Done!
...
9. Build the container:
    ...
    ```
    $ docker build -t edgelab-yolov5 .
    ```

10. Run the container to test the script (you don't need to rebuild the container if you make changes):

    ```
    $ docker run --shm-size=1024m --rm -v $PWD:/scripts edgelab-yolov5 --data-directory data/ --epochs 30 --learning-rate 0.00001 --out-directory out/
    ```
